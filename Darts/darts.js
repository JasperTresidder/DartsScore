...
function Merge() {
  ...
  temp = 0;
  for (k = 0; k < P2LegS.length; k++) {
    temp = temp + int(P2LegS[k]);
  }
  P2LegAv = float(temp / P2LegS.length);
  P2LegAv = round(100 * P2LegAv);
  P2LegAv = P2LegAv / 100;

  temp = 0;
  for (k = 0; k < P1LegS.length; k++) {
    temp = temp + int(P1LegS[k]);
  }
...
function updateScore() {
  ...
  if (lastWin == true) {
    ...
    text('P2 Leg AVR: ' + P2LegAv, 2 * width / 4, 2 * height / 4 + 80);
  } else if (lastWin == false) {
    text(str(P1av), w / 8 - w / 32, 3 * h / 20);
    text('P1 Leg AVR: ' + P1LegAv, 2 * width / 4, 2 * height / 4 + 50);
  } else {
    if (isNaN(P1av) == true) {
      text('0', w / 8 - w / 32, 3 * h / 20);
      if (isNaN(P1LegAv) == true) {
        text('P1 Leg AVR: ' + '0', 2 * width / 4, 2 * height / 4 + 50);
      }else{
        text('P1 Leg AVR: ' + P1LegAv, 2 * width / 4, 2 * height / 4 + 50);
      }
    } else {
    ...
    if (isNaN(P2av) == true) {
      ...
      if (isNaN(P2LegAv) == true) {
        text('P2 Leg AVR: ' + '0', 2 * width / 4, 2 * height / 4 + 80);
      }else{
        text('P2 Leg AVR: ' + P2LegAv, 2 * width / 4, 2 * height / 4 + 80);
      }
    } else {
  ...
  text('P1 last Leg AVR: '+ P1LastLeg, 2 * width / 4, 2 * height / 4 + 130);
  text('P2 last Leg AVR: '+P2LastLeg, 2 * width / 4, 2 * height / 4 + 160);
  lastWin = null;
  fill(0);
}
...
function Undo() {
  if (Turn == true) {
    ...
    if (P1Leg.length != 0) {
      ...
      temp = 0;
      for (k = 0; k < P2Leg.length; k++) {
        temp = temp + int(P2Leg[k]);
      }
      P2LegAv = float(temp / P2Leg.length);
      P2LegAv = round(100 * P2av);
      P2LegAv = P2LegAv / 100;

      temp = 0;
      for (k = 0; k < P1Leg.length; k++) {
        temp = temp + int(P1Leg[k]);
      }
      P1LegAv = float(temp / P1Leg.length);
      P1LegAv = round(100 * P1av);
      P1LegAv = P1LegAv / 100;
      updateScore();
    }
  }
...
function drawScore() {
  ...
  for (var i = 0; i < fin.length; i++) {
    if (P1Leg[P1Leg.length - 1] == int(fin[i][0])) {
      push();
      stroke(0, 255, 0);
      textAlign(CENTER);
      textSize(12);
      text(str(fin[i][1]), w / 4, height / 16);
      pop();
    }
  }
  ...
  for (var i = 0; i < fin.length; i++) {
    if (P2Leg[P2Leg.length - 1] == int(fin[i][0])) {
      push();
      stroke(0, 255, 0);
      textAlign(CENTER);
      textSize(12);
      text(str(fin[i][1]), 3 * w / 4, height / 16);
      pop();
    }
  }
